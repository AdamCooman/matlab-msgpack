...
function [obj, idx] = parse(bytes)
arguments
    bytes (:,1) uint8
end
[obj, idx] = parse_local(bytes, 1);
end

function [obj, idx] = parse_local(bytes, idx)
currentbyte = bytes(idx);
if bitand(0b10000000, currentbyte) == 0b00000000
    % decode positive fixint
    obj = typecast(currentbyte, 'int8');
    idx = idx + 1;
    return
elseif bitand(0b11100000, currentbyte) == 0b11100000
    % decode negative fixint
    obj = typecast(currentbyte, 'int8');
    idx = idx + 1;
    return
elseif bitand(0b11110000, currentbyte) == 0b10000000
    % decode fixmap
    len = double(bitand(0b00001111, currentbyte));
    [obj, idx] = parsemap(len, bytes, idx+1);
    return
elseif bitand(0b11110000, currentbyte) == 0b10010000
    % decode fixarray
    len = double(bitand(0b00001111, currentbyte));
    [obj, idx] = parsearray(len, bytes, idx+1);
    return
elseif bitand(0b11100000, currentbyte) == 0b10100000
    % decode fixstr
    len = double(bitand(0b00011111, currentbyte));
...
out = cell(1, len);
for n=1:len
    [out{n}, idx] = parse_local(bytes, idx);
end
end
...
out = containers.Map();
for n=1:len
    [key, idx] = parse_local(bytes, idx);
    [out(key), idx] = parse_local(bytes, idx);
end
end
